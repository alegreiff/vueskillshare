    ...
    <div class="col-sm-12">
      ...
      <table class="table table-sm" v-for="(orders, index) in getOrders" :key="orders['.key']">
        ...
        <tbody>
          ...
          <tr v-for="(orderItems, index) in orders['.value']">
...
export default {
  ...
  computed: {
    ...mapGetters(['getMenuItems', 'numberOfOrders', 'getOrders', 'currentUser'])
  },
  methods: {
    removeMenuItem(key){
  ...
  beforeRouteLeave: (to, from, next) => {
    if (confirm("recuerde logout") == true) {
      next();
    } else {
      next(false)
    }
